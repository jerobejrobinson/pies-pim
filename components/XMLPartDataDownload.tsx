  ...
  const convertSupabaseToOriginal = (supabaseData: any[]) => {
    return {
      "PIES": {
        ...
        "Items": { 
          "Item": supabaseData.map((part: any) => {
            const descriptions = Array.isArray(part.descriptions) 
              ? part.descriptions.map((desc: any) => ({
                  "#text": desc._text,
            ...
            const partInterchangeGrouped = (Array.isArray(part.partinterchange) ? part.partinterchange : [])
              .reduce((acc: any, interchange: any) => {
            ...
            const digitalFileInfo = Array.isArray(part.digitalfileinformation)
              ? part.digitalfileinformation.map((file: any) => ({
            ...
            const packageData = Array.isArray(part.packages) 
              ? part.packages.map((pkg: any) => ({
              ...
              : part.packages 
                ? [{
                    "QuantityofEaches": part.packages.quantityofeaches,
                    "PackageUOM": part.packages.packageuom,
                    "Dimensions": {
                      "MerchandisingHeight": part.packages.merchandisingheight,
                      "MerchandisingWidth": part.packages.merchandisingwidth,
                      "MerchandisingLength": part.packages.merchandisinglength,
                      "ShippingHeight": part.packages.shippingheight,
                      "ShippingWidth": part.packages.shippingwidth,
                      "ShippingLength": part.packages.shippinglength,
                      "@_UOM": part.packages._uom
                    },
                    "Weights": {
                      "Weight": part.packages.weight,
                      "DimensionalWeight": part.packages.dimensionalweight,
                      "@_UOM": part.packages._uom_weight
                    },
                    "@_MaintenanceType": part.packages._maintenancetype
                  }]
                : [];
